...
func (hr *httpRequest) Get(url string) (http.Response, error) {
	const op = directoryPath + "http_request_service.Get" // Operation identifier for logging

	req, err := http.NewRequest("GET", url, nil) // Create a new GET request
	if err != nil {
		log.Println(op, ": ", err) // Log any errors encountered while creating the request
	...
	if err != nil {
		log.Println(op, ": ", err) // Log any errors encountered during the request
